...
if($_GET['acao'] == 1){
    if($_POST){
        ...
        echo "<script>alert('Registro inserido com sucesso');location.href='atores.php?acao=2'</script>";
    }
}elseif($_GET['acao'] == 3){

    // Consulta os dados do usu�rio com id 1 e privilegio A
    $sql = "SELECT * FROM ator WHERE cod = ?";
    ...
    if($_POST) {
        ...
        echo "<script>alert('Registro atualizado com sucesso');location.href='atores.php?acao=2'</script>";
    ...
    echo "<script>alert('Registro excluido com sucesso');location.href='atores.php?acao=2'</script>";
...
<center>
    ...
    <?php endif; ?>
        <table cellpadding="5">
            <tr>
                <td>ATOR:</td>
                <td><input type="text" name="ator" value="<?= ($_GET['acao'] == 3) ? $dados->nome : NULL ?>"></td>
            </tr>
            <tr>
