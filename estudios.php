...
$crud = Crud::getInstance($pdo, 'estudio');
...
if($_GET['acao'] == 1){
    if($_POST){
        ...
        echo "<script>alert('Registro inserido com sucesso');location.href='estudios.php?acao=2'</script>";
    }
}elseif($_GET['acao'] == 3){

    // Consulta os dados do usu�rio com id 1 e privilegio A
    $sql = "SELECT * FROM estudio WHERE cod = ?";
    ...
    if($_POST) {
        ...
        echo "<script>alert('Registro atualizado com sucesso');location.href='estudios.php?acao=2'</script>";
    ...
    echo "<script>alert('Registro excluido com sucesso');location.href='estudios.php?acao=2'</script>";
    ...
    <form name="form" method="post" action="?acao=3">
        ...
        <form name="form" method="post" action="?acao=1">
            ...
            <table cellpadding="5">
                <tr>
                    <td>ESTUDIO:</td>
                    <td><input type="text" name="estudio" value="<?= ($_GET['acao'] == 3) ? $dados->nome : NULL ?>"></td>
                </tr>
                <tr>
